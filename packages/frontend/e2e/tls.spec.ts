...
test.describe('TLS Certificates', () => {
  ...
  test('TLS page renders certificate table', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'TLS Certificates', exact: true })).toBeVisible();
    await expect(page.getByRole('cell', { name: 'example.com', exact: true }).first()).toBeVisible();
